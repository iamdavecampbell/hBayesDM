###################
#
# This is a modification of the  hBayesDM package igt_pvl_decay model to account for trial
# dependent choices.  Here the consistency parameter theta(t) = (t/10)^c
# where c is between (-5,5)
#
# Potential problem: Note the stan file must be in the working directory.
#
...
igt_pvl_decay_TrialDept <- function(data          = "choose",
                          niter         = 3000, 
                          nwarmup       = 1000,
                          nchain        = 4,
                          ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    ...
    modelPath <- system.file("exec", "igt_pvl_decay.stan.R")
  }
  ...
  modelName <- "igt_pvl_decay_trialDept"
  ...
  m = stan_model(file="igt_pvl_decay_TD.stan",model_name = modelName)
  print(m)
  

  #m = stanmodels$igt_pvl_decay_trialDept
