...
igt_pvl_delta_TrialDept <- function(data          = "choose",
                          niter         = 3000, 
                          nwarmup       = 1000,
                          nchain        = 4,
                          ncore         = 1, 
  ...
  if (modelRegressor) { # model regressors (for model-based neuroimaging, etc.)
    stop("** Model-based regressors are not available for this model **\n")  #igt_pvl_delta_TrialDept
  } else {
    modelPath <- system.file("exec", "igt_pvl_delta.stan.R")
  }
  ...
  modelName <- "igt_pvl_delta_TrialDept"
  ...
  # Fit the Stan model
    ...
    m = stan_model(file="igt_pvl_delta_TD.stan",model_name = modelName)
    print(m)
